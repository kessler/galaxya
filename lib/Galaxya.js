...
function Galaxya(gossiper) {
	...
	function gossipUpdate(k, v, ttl) {
		...
		var key = Key.fromString(k, self._pathSeparator)
		...
		if (key.path[0] === 'service' && key.path.length > 2) {
			try {
				var service = new Service(v)

				if (!service.address)
					debug('discarding service %s without address', service.name)

				self.registerService(service)
			} catch (e) {
				debug('error getting remote service %s', e)
			}
		}
	}
...
Galaxya.prototype.lookupService = function(name, version) {
	...
	debug('getting service %s', key.path)

	var results = this._index.search(key.path)

	var galaxyaResults = []

	for (var i = 0; i < results.length; i++) {
		galaxyaResults.push(new Service(results[i][1]))
	}

	galaxyaResults.sort(sort)

	if (version)
		galaxyaResults = filterServices(version, galaxyaResults)

	return galaxyaResults
}

Galaxya.prototype.waitForService = function(name, version, callback) {
	if (!name)
	...
	// this returns a filtered and sorted list of services, first one being the highest
	var current = this.lookupService(name, version)

	if (current.length > 0)
		return callback(current[0])

	version = version || '0.0.0'

	var eventName = Service.toEventName(this._pathSeparator, name)

	var self = this

	function listener(service) {

		if (gtOrEq(version, service)) {
			callback(service)
		} else {
...
function gtOrEq(version, service) {
	return semver.satisfies(service.version, '>=' + version)
}

function filterServices(version, services) {
	for (var i = services.length - 1; i >= 0; i--) {
		if (gtOrEq(version, services[i]))
			break
	}
...
Galaxya.filterServices = filterServices
...
Galaxya.prototype.registerService = function(serviceData) {
	...
	debug('adding service %s, %s', rawKey, util.inspect(service))
...
Galaxya.prototype._fireServiceRegistered = function(service) {
	...
	debug(service.toEventName(this._pathSeparator), service.toEventName(this._pathSeparator), true)
	this.emit(service.toEventName(this._pathSeparator), service)
	//this.emit(service.toEventName(this._pathSeparator, true), service)
}
...
Service.prototype.toEventName = function (pathSeparator, withVersion) {

	if (withVersion)
		return Service.toEventName(pathSeparator, this.name, this.version)
	else
		return Service.toEventName(pathSeparator, this.name)
}

Service.toEventName = function(pathSeparator, name, version) {
	var result = 'service' + pathSeparator + name

	if (version !== undefined)
		result += pathSeparator + version
