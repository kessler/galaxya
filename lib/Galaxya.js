var Trie = require('./Trie')
var Key = require('./Key')
var debug = require('debug')('galaxya')
var EventEmitter = require('events').EventEmitter
var util = require('util')
var semver = require('semver')
var assert = require('assert')

module.exports = Galaxya

util.inherits(Galaxya, EventEmitter)
function Galaxya(gossiper) {
	EventEmitter.call(this)

	this._pathSeparator = '/'

	this._gossiper = gossiper

	var index =	this._index = new Trie()

	var self = this

	this._gossiper.on('update', gossipUpdate)
	this._gossiper.on('expired', gossipExpire)
	this._gossiper.on('peer_alive', gossipPeerAlive)
	this._gossiper.on('peer_failed', gossipPeerFailed)

	function gossipUpdate(peer, k, v, ttl) {
		debug('gossip update [%s]', k)

		if (ttl && ttl < Date.now())
			return;

		var key = Key.fromString(self._pathSeparator, k)

		if (!key) {
			return debug('illegal key %s', k)
		}

		if (key.path[0] === 'service' && key.path.length > 2) {
			try {
				var service = self.newService(v)
				self.registerService(service)
			} catch (e) {
				debug('error updating remote service %s', e)
			}
		} else {
			self.emit(key.path[0], new Message(key, v, ttl))
		}
	}

	function gossipExpire(expired) {
		for (var i = 0; i < expired.length; i++) {
			var rawKey = expired[i]

			// TODO uncomment when implementation is done
			//index.remove(Key.fromString(k, self._pathSeparator).path)
			self.emit('expired', rawKey)
		}
	}

	function gossipPeerAlive (peer) {
		debug('peer alive %s', peer.name)
		self.emit(peer.name, { alive: true })
	}

	function gossipPeerFailed (peer) {
		debug('peer failed %s', peer.name)
		self.emit(peer.name, { alive: false })
	}
}

Galaxya.prototype.start = function(callback) {
	this._gossiper.start(callback)
}

Galaxya.prototype.save = function (message) {
	this._index.put(message.key.path, message.value)
	this._gossiper.setLocalState(message.key.toString(), message.value, message.ttl)
}

Galaxya.prototype.onServiceActivity = function(service, fn) {
	this.on(service.gossiper, fn)
}

Galaxya.prototype.onceServiceActivity = function(service, fn) {
	this.once(service.gossiper, fn)
}

//TODO add escape for separators
Galaxya.prototype.registerService = function(serviceData) {

	if (!serviceData.address)
		serviceData.address = serviceData.address || this._gossiper.address

	if (!serviceData.gossiper)
		serviceData.gossiper = this._gossiper.peer_name

	var service = this.newService(serviceData)

	// its a little wasteful to duplicate data both in the key and the value, but it makes life easier
	var key = new Key(['service', service.name, service.version, service.address, service.port], this._pathSeparator)

	var rawKey = key.toString()

	debug('adding service %s', rawKey)

	this._index.put(key.path, service)

	this._gossiper.setLocalState(rawKey, serviceData, service.ttl)

	this._fireServiceRegistered(service)

	return key
}

Galaxya.prototype.lookupService = function(name, version) {
	if (!name)
		throw new Error('must supply a name')

	var key = new Key(['service', name], this._pathSeparator)

	debug('lookup services %s', key.path)

	var results = this._index.searchByPrefix(key.path, true)

	results.sort(sort)

	if (version)
		results = filterServices(version, results)

	return results
}

Galaxya.prototype.waitForService = function(name, version, callback) {
	if (!name)
		throw new Error('must specify a service name')

	if (typeof version === 'function') {
		callback = version
		version = undefined
	}

	if (!callback)
		throw new Error('must supply a callback')

	// this returns a filtered and sorted list of services, first one being the highest
	var current = this.lookupService(name, version)

	if (current.length > 0)
		return callback(current[0])

	version = version || '0.0.0'

	var eventName = Service.toEventName(this._pathSeparator, name)

	var self = this

	function listener(service) {

		if (gtOrEq(version, service)) {
			callback(service)
		} else {
			self.once(eventName, listener)
		}
	}

	this.once(eventName, listener)
}

function gtOrEq(version, service) {
	return semver.satisfies(service.version, '>=' + version)
}

function filterServices(version, services) {
	for (var i = services.length - 1; i >= 0; i--) {
		if (gtOrEq(version, services[i]))
			break
	}

	return services.slice(0, i + 1)
}

Galaxya.filterServices = filterServices

function sort(a, b) {
	assert.ok(a)
	assert.ok(b)

	return semver.lt(a.version, b.version)
}

Galaxya.prototype.newService = function(data) {
	return new Service(data)
}

Galaxya.prototype._fireServiceRegistered = function(service) {

	this.emit(service.toEventName(this._pathSeparator), service)
	//this.emit(service.toEventName(this._pathSeparator, true), service)
}

function Message(k, v, ttl) {
	this.key = k
	this.value = v
	this.ttl = ttl
}

function Service(data) {
	if (!data)
		throw new Error('missing service data (simple hash)')

	if (!data.name)
		throw new Error('missing service name')

	this.name = data.name

	if (!data.port)
		throw new Error('missing service port')

	this.port = data.port.toString()

	if (!data.address)
		throw new Error('missing service address')

	this.address = data.address

	if (!data.gossiper)
		throw new Error('missing gossiper')

	/* the original gossiper from which this service came */
	this.gossiper = data.gossiper

	this.version = data.version || '0.0.0'

	if (!semver.valid(this.version))
		throw new Error('invalid version ' + this.version)

	this.data = data.data || {}
}

Service.prototype.toEventName = function (pathSeparator, withVersion) {

	if (withVersion)
		return Service.toEventName(pathSeparator, this.name, this.version)
	else
		return Service.toEventName(pathSeparator, this.name)
}

Service.toEventName = function(pathSeparator, name, version) {
	var result = 'service' + pathSeparator + name

	if (version !== undefined)
		result += pathSeparator + version

	return result
}